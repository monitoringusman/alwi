...
<table class="datatable" border="0">
		...
		<tr >
			<td>
					...
					Checking for Pygal library (module for python)
					<br />You have 
					<?php $output = shell_exec('pip3 list | grep pygal'); 
						  preg_match('@[0-9]+\.[0-9]+\.[0-9]+@', $output, $version); 
						  if($version[0] !="") {
							$passgd=1;
							echo "Pygal ".$version[0];
						}
						else
						{
								echo "no Pygal library. Please install pip3 and pygal from pip3";
						} 
					?>
					</td>
			<td >
