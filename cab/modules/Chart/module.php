...
function __construct($variables){ // 
  ...
  function writeArraysToFile($userData) #write point arrays to file for external libs
  {
	...
	$fp = fopen($this->vars['root_dir'].'/modules/Chart/data/'.$userData['chartname'].''.$userData['userid'].'_val.txt', 'w');
	...
	$fp = fopen($this->vars['root_dir'].'/modules/Chart/data/'.$userData['chartname'].''.$userData['userid'].'_label.txt', 'w');
  ...
  function drawImagePygal($userData) #рисуем график
  {
	  ...
	  if($userData['charttype']=="line"){
			echo passthru("python3 ".$this->vars['root_dir']."/modules/Chart/pygal/line.py ".$userData['chartname']."".$userData['userid']."");
			echo file_get_contents($this->vars['root_dir'].'/modules/Chart/pictures/'.$userData['chartname'].''.$userData['userid'].'.svg');

	  }

	  if($userData['charttype']=="pie"){
			echo passthru("python3 ".$this->vars['root_dir']."/modules/Chart/pygal/pie.py ".$userData['chartname']."".$userData['userid']."");
			echo file_get_contents($this->vars['root_dir'].'/modules/Chart/pictures/'.$userData['chartname'].''.$userData['userid'].'.svg');
	  }

	  if($userData['charttype']=="gauge"){
		echo passthru("python3 ".$this->vars['root_dir']."/modules/Chart/pygal/gauge.py ".$userData['chartname']."".$userData['userid']."");
		echo file_get_contents($this->vars['root_dir'].'/modules/Chart/pictures/'.$userData['chartname'].''.$userData['userid'].'.svg');
  }
